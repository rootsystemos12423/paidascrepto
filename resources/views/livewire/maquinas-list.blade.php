<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        @forelse($maquinas as $machine)
            <div class="bg-white shadow-md rounded-lg p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div>
                    <h2 class="text-lg font-semibold">{{ $machine->nome }}</h2>
                    <div class="flex gap-2 mt-2">
                        <p class="text-white bg-gray-400 px-2 py-1 text-xs text-center text-center flex items-center rounded-full">{{ $machine->algoritmo }}</p>
                        <p class="text-white bg-gray-400 text-center flex items-center px-2 py-1 text-xs text-center rounded-full">{{ $machine->maquina_modelo }}</p>
                    </div>
                </div>
                
                <div class="progress-bar flex space-x-1 w-full md:w-auto mt-4 md:mt-0">
                    <!-- Barra de progresso simulada -->
                    @for($i = 0; $i < 10; $i++)
                        <div class="bar w-full md:w-4 bg-green-500 h-2 rounded-md"></div>
                    @endfor
                </div>
